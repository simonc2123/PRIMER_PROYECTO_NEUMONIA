...
def test_preprocess_image():
    ...
    assert result.shape == (1, 512, 512, 1), f"Forma esperada (1, 512, 512, 1) obtenida {result.shape}"
...
def test_preprocess_normalization():
    ...
    assert np.all(result >= 0) and np.all(result <= 1), "Los valores de píxeles no están normalizados en el rango [0, 1]"
