    ...
    try:
        ...
        if save_csv:
            with open(HISTORIAL_FILE, "a", newline="", encoding="utf-8") as f:
                ...
                writer.writerow(
                    [fecha_hora, patient_id or "N/A", label, f"{probabilidad:.2f}%"]
                )
...
def interactive_mode():
    ...
    while True:
        ...
        if choice == "1":
            ...
            patient_id = (
                input("👤 Cédula del paciente (Enter para omitir): ").strip() or None
            )
            ...
            if HISTORIAL_FILE.exists():
                confirm = input(
                    "⚠️  ¿Seguro que desea limpiar el historial? (s/n): "
                ).lower()
                if confirm == "s":
...
Formatos soportados: DICOM (.dcm), JPEG (.jpg, .jpeg), PNG (.png)
    ...
    parser.add_argument("image", nargs="?", help="Ruta a la imagen de rayos X")
    ...
    parser.add_argument(
        "-p", "--paciente", metavar="ID", help="Cédula o ID del paciente"
    )
    ...
    parser.add_argument(
        "-v", "--version", action="version", version="%(prog)s 2.0 - Equivalente a GUI"
    )
    ...
    try:
        if args.interactive:
            ...
            return analyze_image(
                args.image, args.paciente, args.heatmap, args.csv, args.pdf
            )
        else:
